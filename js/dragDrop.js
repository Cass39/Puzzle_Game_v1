...
class DragDrop {
  ...
  dragDropEvents() {
    const { draggableDivs, puzzleDivs, modal, modalText,
      modalBtn, attempt, cellsAmount } = this.positionElements.elements
    ...
    draggableDivs.forEach((draggableDiv, i) => {
      ...
      puzzleDivs[i].addEventListener("drop", (e) => {
         ...
         this.selected.style.left = 0
          ...
          if(this.selected.dataset.index === puzzleDivs[i].dataset.index) {
            ...
            puzzleDivs.forEach((div) => {
              div.firstElementChild && div.dataset.index 
              === div.firstElementChild.dataset.index &&
              this.points.correct++
            })
          } else {
            this.points.wrong++
